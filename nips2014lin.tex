...
\textbf{TODO:} Tidy the following paragraph up
...
\textbf{TODO}: Learning these GPs 
\begin{equation}
    \Fengy \approx - \frac{1}{2} \bigg[
        N\log{2\pi\lvar} + \log\deter{\KERNL} 
        - \log\deter{\pocov}
    + \pomean\transpose\KERNL\inv \pomean
    + \frac{1}{\lvar}\brac{\obs - \bar\obs}\transpose\!\brac{\obs - \bar\obs}
        \bigg].
    \label{eq:fengygp}
\end{equation}
...
\begin{algorithm}[htb]
    \SetAlgoLined
    \While{$\Fengy_\text{new} - \Fengy_\text{old} > \varepsilon$}{
