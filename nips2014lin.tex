...
\begin{itemize}
    ...
    \item Ultimately we end up with an algorithm (UGP) that can use
        ``black-box'' likelihood functions where the algorithm does need a
        derivative, or the likelihood may not even be differentiable.
\end{itemize}
...
\subsection{Learning the Parameters and Hyperparameters}
...
and kernel hyperparameters, $\kfunc{\cdot}{\cdot|\khypers}$.

A line search could be used to select an optimal value for the step length,
$\step$ in \eqref{eq:pomeangp}. However, we find that setting $\step=1$, and
then successively multiplying $\step$ by some number in $\brac{0, 1}$ until the
MAP objective \eqref{eq:MAP} lowers, or some maximum number of iterations is
exceeded, works well in practice. If the maximum number of iterations is
exceeded we call this a `diverge' condition, and terminate the search of
$\pomean$ (and return the last good value). This only tends to happen for the
unscented GP, and does not tend to impact the algorithms performance, as
demonstrated in \autoref{fig:learnex}. The dominant computational cost in
learning the parameters is the inversion in \eqref{eq:pocovgp}, and so the
...
any potential implicit gradient terms. 
...
scientific python (scipy.integrate.quad) for quadrature.
...
inversion problems. We generate `latent' function data from
$\lstate_{\text{true}}\!\brac{\inobs} = \sin\!\brac{\inobs} +
\cos\!\brac{\pi\inobs}$, where $\inobs$ are drawn randomly from a uniform
distribution between $[-2\pi, 2\pi]$. Observations used to test and train the
GPs are then generated as $\obs = \nonlin{\lstate_\text{true}} + \epsilon$
where $\epsilon \sim \gaus{0, 0.2^2}$. 500 points are generated in this manner,
and we use 5-fold cross validation to train (4/5 th) and test (1/5 th) the GPs.
We standardised mean squared error (SMSE) to test the predictions with the held
...
$-\frac{1}{N\test} \sum_{n} \log \gausC{\lstates_n\test}{\pomeans_n\test,
    \pocovs_n\test}$. All of the GPs use Matern $\frac{5}{2}$ covariance
...
\autoref{fig:learnex} we show the results of the UGP using a forward
model for which no derivative exists. We also show a typical objective function
trace for learning the posterior mean which terminated on a `divergence'
condition. We use quadrature for the predictions in observations space in
\autoref{tab:toy} and the unscented transform for the predictions in
\autoref{fig:learnex}.
...
algorithms. We test against derived-for-the-task Laplace and expectation
propagation GP classifiers (from the GPML matlab toolbox), a support vector
machine (with probabilistic outputs), and logistic regression (both from the
scikits-learn python library). We report performance as the average negative
log-likelihood of a Bernoulli distribution, $ - \frac{1}{N\test}$, and also
error rate as the percent incorrectly misclassified labels in the test set
(once the probabilistic outputs have been thresholded at 0.5). 
...
\begin{itemize}
    ...
    \item Although performance is great, we do find the UGP and EGP sensitive
        to starating values of the hyperparameters, and especially the
        likelihood noise, $\lvar$, if it is not initialised to a small value.
\end{itemize}
