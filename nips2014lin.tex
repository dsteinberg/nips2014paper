...
posterior mean. No such guarantees exist for statistical linearization, though
monitoring \eqref{eq:MAP} works well in practice (see the experiment in
...
and we use 5-fold cross validation to train (400) and test (100) the GPs.  We
use standardised mean squared error (SMSE) to test the predictions with the
held out data in both the latent and observed spaces. We also use held-out
average negative log-likelihood on the latent test data, which is calculated as
...
`divergence' condition. We use quadrature for the predictions in observations
...
optimised values are near these values). The hyperparameters for the support
