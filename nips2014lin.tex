...
any implicit gradient terms between $\lstate$ and~$\khypers$. 
...
\begin{figure}[tb]
    ...
    \caption[]{Example of learning the unscented Gaussian process (UGP) with a
        ...
        MAP objective function used to learn $\pomean$ from this nonlinear
        function is shown in (\subref{sub:mape}). The optimization shown
        terminated because of a `divergence' condition.}
...
transform for the predictions in \autoref{fig:learnex}. Interestingly, there
is almost no difference in performance between the EGP and UGP, even though the
...
get stuck in local minima for some large initial values of $\lvar$, e.g.~$1.0$.
