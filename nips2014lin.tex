...
forward model. Unfortunately the marginal likelihood $\prob{\obs}$ is intractable
...
%    \label{eq:marglike}
%\end{equation}
which also makes the posterior $\probC{\lstate}{\obs}$ intractable to evaluate. Thus, we choose to
approximate the posterior % and marginal likelihood 
...
process as described in  \autoref{sec:gpmod}. When optimizing $\pomean$, the
