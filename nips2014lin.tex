...
\begin{figure}[tb]
    ...
    \caption[]{Example of learning the unscented Gaussian process (UGP) with a non-differentiable nonlinear
        function in (\subref{sub:sign}). In this figure we show the predictive
        distribution of $\expec{q\lstates\test}{\obss\test}$ using
        Equation \eqref{eq:slpred}. A typical trace from the MAP objective function used
        to learn $\pomean$ from the nonlinear function in (\subref{sub:sign})
        is shown in (\subref{sub:mape}). The optimization shown terminated
...
\textbf{TODO:} Discuss the experiment results w.r.t.\ \cite{Opper2009}. \marginpar{FIX}


\subsection{Binary Handwritten Digit Classification}

For this experiment we evaluate the EGP and UGP on a %more routine 
binary
...
vector machine (SVM) with a radial basis kernel function (and probabilistic outputs
\cite{Platt1999}), and logistic regression (both from the scikit-learn python
library \cite{scikit-learn}).  We report  the average negative
...
%classifier. 
and the error rate. %as the percentage of incorrectly
%misclassified labels in the test set (once the probabilistic outputs have been
%thresholded at $0.5$). 
A square exponential kernel with amplitude
$\sigma_\text{se}$ and length scale $l_\text{se}$ is used for the GPs
in this experiment. We initialize these hyperparameters at $1.0$, and
put a lower bound of $0.1$ on them. We initialize $\lvar$ at $10^{-12}$ in the
EGP and UGP, and place a lower bound at $10^{-14}$ (the optimized values are
near these values). The hyperparameters for the SVM are
learned using grid search with three-fold cross validation. 

The results are summarized in \autoref{tab:class}, along with learned
...
% 
% Withheld for review.
% EVB would like to thank his wife, daughter and cat for coping with his absence at hoe :-)
